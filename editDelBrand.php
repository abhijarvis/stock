...
<form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9"> 
      <input type="text" class="form-control" name="brand_name" id="brand_name" value="<?php if(!empty($row)) { echo $row['brand_name']; } ?>">
...
<?php
	if ((empty($row['brand_id']))){
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" disabled>Update brand</button>';
	} else{
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" >Update brand</button>';
	}
?>
...
<form class="form-inline" action="php_action/deleteBrand.php" method="POST">
 ...
 <?php
	if ((empty($row['brand_id']))){
		echo '<button style="margin: 30px 0 30px 10px;" type="submit" class="btn btn-primary" disabled>Delete Brand</button>';
	} else{
		echo'<button style="margin: 30px 0 30px 10px;" type="submit" class="btn btn-primary">Delete Brand</button>';
	}
?>
