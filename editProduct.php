...
<form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
  ...
  <label class="control-label col-sm-2" for="productName">Product Name:</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="product_name" name="product_name" placeholder="Enter product name" value="<?php if(!empty($row)) { echo $row['product_name'];}?>" required>
    </div>
  </div>
   ...
   <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="rate" name="rate" placeholder="Rate (price per piece)" value="<?php if(!empty($row)) { echo $row['rate'];}?>" required>
    </div>
  </div>
...
<?php
	if ((empty($row['product_id']))){
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" disabled>Update Product Detail</button>';
	} else{
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" >Update Product Detail </button>';
	}
?>
