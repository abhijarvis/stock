...
<form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9"> 
      <input type="text" class="form-control" name="category_name" id="category_name" value="<?php if(!empty($row)) { echo $row['category_name'];} ?>">
    </div>
  </div>
...
<?php
	if ((empty($row['category_id']))){
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" disabled>Update category </button>';
	} else{
		echo '<button style="margin: 30px 0 30px 0;float:left;" type="submit" class="btn btn-success" >Update category</button>';
	}
?>
...
<?php
	if ((empty($row['category_id']))){
		echo '<button style="margin: 30px 0 30px 10px;" type="submit" class="btn btn-primary" disabled>Delete category </button>';
	} else{
		echo '<button style="margin: 30px 0 30px 10px;" type="submit" class="btn btn-primary" >Delete category</button>';
	}
?>
